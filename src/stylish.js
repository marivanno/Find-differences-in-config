...
const treeStile = (data, depth = 0) => {
  const result = data.reduce((acc, item) => {
    if (item.type === 'added' && _.isObjectLike(item.value)) {
      ...
      + ${item.key}: ${buildStringTree(buildObjectToArray(item.value), 2)}`;
    }
    if (item.type === 'added') {
      return `${acc} \n${spacegenerator(depth)}
      + ${item.key}: ${item.value}`;
    ...
    if (item.type === 'modifed' && _.isObjectLike(item.valueAfter)) {
      return `${acc} \n${spacegenerator(depth)}
      - ${item.key}: ${buildStringTree(buildObjectToArray(item.valueAfter), 2)}\n${spacegenerator(depth)}
      + ${item.key}: ${item.valueBefore}`;
    }
    if (item.type === 'modifed' && _.isObjectLike(item.valueBefore)) {
      return `${acc} \n${spacegenerator(depth)}
      - ${item.key}: ${item.valueAfter}\n${spacegenerator(depth)}
      + ${item.key}: ${buildStringTree(buildObjectToArray(item.valueBefore), 2)}`;
    }
