...
test.each([
  ...
  const file1 = path.resolve('', `__tests__/__fixtures__/${before}`);
  const file2 = path.resolve('', `__tests__/__fixtures__/${after}`);
  const ast = buildAst(file1, file2);
  fs.writeFileSync(`c:/dev/${format}_${before}_${after}_result`, formatter(ast, format), 'utf8');
  const result = fs.readFileSync(path.resolve('', `__tests__/__fixtures__/${format}_${before}_${after}_result`), 'utf8');
